...
func loadConfig(configPath string, outDir string) (loadedConfig, error) {
	...
	if len(root.Playout.PlaylistOrder) == 0 {
		root.Playout.PlaylistOrder = []string{"current_conditions", "air_quality", "forecast", "climate_summary", "thunderstorm_outlook", "metnotes", "hydrometric", "coastal_flood", "hurricane_tracks", "precipitation_analysis", "eccc_discussion", "geophysical_alert", "user_bulletin"}
	}
